...
- [x] 1:1 chat UX: **encrypted media attachments** (encrypt → IPFS-style store → decrypt → render; per-blob key inside the E2EE frame), **self-destruct timers**, **typing indicators**, **double-tap reactions**, and **reply-with-quote** — all **verified in Chromium** (`web/e2e/{media,interactions,reply}.spec.ts`). Voice messages remain.
