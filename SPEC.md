...
- [x] 1:1 chat UX: **encrypted media attachments** (encrypt → IPFS-style store → decrypt → render; per-blob key inside the E2EE frame), **self-destruct timers**, **typing indicators**, and **double-tap reactions** — all **verified in Chromium** (`web/e2e/media.spec.ts`, `interactions.spec.ts`). Voice messages remain.
