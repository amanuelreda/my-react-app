...
- [x] 1:1 chat UX: **encrypted media attachments**, **voice messages** (same encrypted media path; playable `<audio>` from decrypted bytes), **self-destruct timers**, **typing indicators**, **double-tap reactions**, **reply-with-quote**, and **in-chat search** — all **verified in Chromium** (`web/e2e/{media,voice,interactions,reply,chat-search}.spec.ts`).
...
- [x] Interactive forums in the client: reputation-weighted up/down voting and new-thread posting that mutate the in-browser indexer (live re-ranking) and build on-chain `ForumManager` calldata (encoders unit-tested) — **verified in Chromium** (`web/e2e/forum-interact.spec.ts`).
- [ ] Swap the in-browser seed for live indexer `fetch()`/subgraph queries against a deployed testnet.
...
7. **Anchored "proof of conversation."** Optional Merkle-root anchoring lets any party later prove a message existed at a time/order without revealing content — useful for disputes, agreements, or journalism, while staying private. *Implemented & tested:* `shared/src/anchor.js` (`frameHash`/`buildMerkleRoot`/`merkleProof`/`verifyMerkleProof`, domain-separated BLAKE2b) feeds `GroupManager.appendCommitment()`; 6 tests cover leaf verification, tamper-evidence, reordering, and edge cases.
