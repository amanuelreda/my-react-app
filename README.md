...
- **[`docs/`](./docs)** — [`STATUS.md`](./docs/STATUS.md) (spec→code→test traceability matrix),
  [`RUNBOOK.md`](./docs/RUNBOOK.md) (testnet→mainnet go-live), [`THREAT_MODEL.md`](./docs/THREAT_MODEL.md),
  and architecture ADRs.
...
| Contracts compile (solc) + **EVM behavior tests** (no Foundry) | ✅ `cd contracts && npm install && npm run compile && npm run test:evm` |
| Core contracts + Foundry fuzz/invariant tests | ✅ in CI (`forge test`) |
...
| Interactive forums: voting + posting + replies (browser-verified) | ✅ Playwright |
| Chat→forum crosspost + group creation (browser-verified) | ✅ 25 Playwright tests |
